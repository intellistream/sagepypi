...
def test_version_source_contract():
    ...
    assert attr == "sagepypi._version.__version__"

    version_file = root / "src" / "sagepypi" / "_version.py"
    init_file = root / "src" / "sagepypi" / "__init__.py"
    ...
    patterns = [
        r"^from\s+sagepypi\._version\s+import\s+.*__version__",
